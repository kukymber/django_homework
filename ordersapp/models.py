...
class Order(models.Model):
    ...
    updated = models.DateTimeField(verbose_name='Обнoвлен', auto_now_add=True)
    paid = models.DateTimeField(verbose_name='оплачен')
    status = models.CharField(choices=OREDER_STATUS_CHOICES, verbose_name='статус', max_length=3, default=FORMING)
    ...
    def get_total_quantity(self):
        items = self.orderitems.select_related()
        return sum(list(map(lambda x: x.get_product_quantity(), items)))
...
class OrderItem(models.Model):
    ...
    @staticmethod
    def get_item(pk):
        return OrderItem.objects.get(pk=pk).qunatity
